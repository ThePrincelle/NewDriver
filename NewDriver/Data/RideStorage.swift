...
class RideStorage: ObservableObject {
    ...
    private let ridesRecordID: CKRecord.ID
    ...
    init(isTesting: Bool = false) {
        ...
        ridesRecordId = CKRecord.ID(recordName: isTesting ? "ridesTest" : "rides")
        getLastPerson()
    }
    ...
    // Deletes the last person record.
    /// - Parameter completionHandler: An optional handler to process completion `success` or `failure`.
    func deleteRides(completionHandler: ((Result<Void, Error>) -> Void)? = nil) {
        database.delete(withRecordID: lastPersonRecordID) { recordID, error in
            if let recordID = recordID {
                os_log("Record with ID \(recordID.recordName) was deleted.")
            }
    ...
    func getRides(completionHandler: ((Result<Void, Error>) -> Void)? = nil) {
        database.fetch(withRecordID: ridesRecordId) { record, error in
            if let record = record {
                os_log("Record with ID \(record.recordID.recordName) was fetched.")
                if let name = record["rides"] as? [Ride] {
                    DispatchQueue.main.async {
                        self.rides = rides
                    }
                }
